...
int a[4][4], check = 0;
// Ham di chuyen cac o so sang trai
...
// Ham di chuyen cac o so sang phai
...
// Ham di chuyen cac o so len tren
...
// Ham di chuyen cac o so xuong duoi
...
int state = 0;
int diem = 0;
//Ham xu ly su kien khi nhan phim
int getKey()
{
...
//Ham random so va vi tri cua so trong man hinh game
void randPosNum(int Val[][4], int check)
{
...
{
	fstream fileScore;
	//Mo file de luu gia tri diem so hien tai
...
int game() {
	while (true)
	{
		...
		if (checkGame(Val) == 0)
		{
			...
			textcolor(7);
			gotoxy(30, 14);
			cout << "Chon y de choi luot choi moi";
			gotoxy(30, 15);
			cout << "Nhan phi bat ki de thoat\n";
			char c = _getch();
			if (c == '1') {
				return 1;
			}
